...
const UserInfo: React.FC<UserInfoProps> = (props) => {
  ...
  const toggleMoreInfo = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    btn.innerHTML = btn.innerHTML === "Показать подробную информацию"
    ...
    <div className={s.container}>
      <div className={s.info_wrapper}>
        <div className={s.user_name}>
          <p>
            {props.profile.fullName} <span className={s.status}>online</span>
          </p>
        ...
        {moreInfo ? (
          <AboutUser isOwner={props.isOwner} profile={props.profile} />
        ) : null}
      </div>
...
const AboutUser: React.FC<AboutUserProps> = (props) => {
  ...
  const formSubmitted = (formData: any) => {
